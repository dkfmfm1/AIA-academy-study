...
public class Manager {
	...
	void showAllData() {
		...
		for(int i=0; i<cnt; i++) {
			memberNum[i].showInfor();
			System.out.println("-----------------------------");
		}
		
	}
	
	// 배열에서 아이디를 기준으로 검색 후 index 값을 반환
	...
	// 검색 : 아이디를 기준으로 데이터를 찾아서 해당 데이터의 정보를 출력
	// 배열의 요소(PhoneInfor)의 id 속성 값으로 배열의 요소를 찾음
	void searchInfo() {
		...
		if(searchIndex<0) {
			...
			memberNum[searchIndex].showInfor();
		}
	...
	void loginInfo() {
		...
		System.out.println("아이디를 입력해주세요.");
		String checkName1 = sc.nextLine();
		System.out.println("비밀번호를 입력해주세요.");
		String checkName2 = sc.nextLine();
		
		if(true) {
			System.out.println("로그인 되었습니다.");
		} else {
			System.out.println("회원정보가 없습니다. 회원가입해주세요.");
		}
		
		
	}
	
	// 삭제: id을 기준으로 데이터를 찾아서 해당 데이터를 삭제
	void deleteInfo() {
